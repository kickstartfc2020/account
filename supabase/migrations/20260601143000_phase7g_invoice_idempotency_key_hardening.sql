-- Phase 7g: Harden invoice idempotency against retries/timeouts/refresh.
-- Adds durable idempotency key on invoices and uses it in finalize_invoice_write.

alter table public.invoices
  add column if not exists idempotency_key text;

create unique index if not exists uq_invoices_org_idempotency_key
  on public.invoices(organization_id, idempotency_key)
  where idempotency_key is not null;

-- Backfill idempotency key from invoice_write_requests where mapping exists.
update public.invoices inv
set idempotency_key = iwr.request_key
from public.invoice_write_requests iwr
where iwr.invoice_id = inv.id
  and inv.idempotency_key is null;
...
begin
  ...
  if p_invoice_number is not null and btrim(p_invoice_number) <> '' then
    ...
    -- Secondary lookup directly on invoices, for durable idempotency even if request mapping is stale.
    select inv.id, inv.invoice_number
      into v_created_invoice_id, v_created_invoice_number
    from public.invoices inv
    where inv.organization_id = v_organization_id
      and inv.idempotency_key = v_request_key;
