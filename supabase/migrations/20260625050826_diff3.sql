...
CREATE OR REPLACE FUNCTION public.get_financial_year_reset_preview(p_organization_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(financial_year text, fy_start date, fy_end date, invoices_count integer, invoice_items_count integer, payments_count integer, renewals_count integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
