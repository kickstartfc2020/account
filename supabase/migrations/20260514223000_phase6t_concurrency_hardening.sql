-- Phase 6t: Concurrency and race-condition hardening for accounting flows.
...
create index if not exists idx_invoice_write_requests_invoice_id
  on public.invoice_write_requests(invoice_id);

alter table public.invoice_write_requests enable row level security;

drop policy if exists invoice_write_requests_deny_all on public.invoice_write_requests;
create policy invoice_write_requests_deny_all
on public.invoice_write_requests
as restrictive
for all
using (false)
with check (false);

drop trigger if exists invoice_write_requests_set_updated_at on public.invoice_write_requests;
create trigger invoice_write_requests_set_updated_at
before update on public.invoice_write_requests
